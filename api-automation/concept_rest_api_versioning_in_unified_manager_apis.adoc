...
이 버전 번호의 숫자 부분은 릴리스에 대해 증분됩니다. 버전 번호가 있는 URI는 향후 릴리스에서 이전 버전과의 호환성을 유지하는 일관된 인터페이스를 제공합니다. 또한 버전 없이 동일한 API를 찾을 수도 있습니다 `/datacenter/svm/svms,` 버전을 사용하지 않는 기본 API를 나타냅니다. 기본 API는 항상 최신 버전의 API입니다.
