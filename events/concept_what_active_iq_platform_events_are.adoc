...
플랫폼 이벤트 활성화에 대한 자세한 내용은 을 참조하십시오 link:../config/concept_active_iq_platform_events.html["Active IQ 포털 이벤트 활성화"].
규칙 파일 업로드에 대한 자세한 내용은 을 참조하십시오 link:../events/task_upload_new_active_iq_rules_file.html["새 Active IQ 규칙 파일을 업로드하는 중입니다"].
